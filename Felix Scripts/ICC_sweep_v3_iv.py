...
SavePlace='E:\\transfer\\CC\\Stephan\\MT31\\X01Y04D2_100nm\\'
import datetime
try:
    os.mkdir(SavePlace)
...
ps.range['B']=2
ps.offset['B']=2
d=picoiv(tri(2, -VRESET/10), duration=cycleduration, n=100, fs=1.25e7, smartrange=True, savewfm=True)

####
filename = datetime.datetime.fromtimestamp(time.time()).strftime('%Y_%m_%d_%H_%M_%S')+'_ICC_'+str(ICC)+'_VRESET_'+str(10*VRESET)+'duration'+str(cycleduration)
meta=io.MetaHandler()
