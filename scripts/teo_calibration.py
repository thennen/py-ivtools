...
def teo_calibration(V_sweep=9, sweep_rate=100_000, resistor=47_250, LF_nplc=10, digipot=False, test=False,
                    save_folder="X:\emrl\Pool\Bulletin\Handbücher.Docs\TS_Memory_Tester\calibration", plotit=True,
                    update_project=True, plot_live=True):
    """
    TeoSystem calibration using Digipot, Picoscope and Keitheley.

    Parameters
    -----------
    V_sweep: Amplitude of the sweep used
    sweep_rate: Sweep rate used
    resistor: resistance smapled for calibration
    LF_nplc: Number of power cycles used for LF calibration
    digipot: If you are using digipot or not
    test: If you are calibrating or testing the existing calibration
    save_folder: Folder in which to save all data
    plotit: If to plot results or not
    update_project: If Update the calibration file in the project or not
    ...
    if digipot:
        ...
        if isnumber(digipot) and resistor == 0:
            R = dp.set_R(digipot)
        else:
            dp.set_R(0)
    else:
    ...
    def ps_interpreter(datain, dtype=np.float32):
        '''
    ...
    if check is True:
        if os.path.isfile(settings.teo_calibration_file):
            teo.calibration = pd.read_pickle(settings.teo_calibration_file)
    ...
    if check is False:
        if os.path.isfile(settings.teo_calibration_file):
            teo.calibration = pd.read_pickle(settings.teo_calibration_file)  # so you don't get crazy trying to
            # apply calibration later
        if update_project is True:
            calibration.to_pickle('ivtools/instruments/calibration/teo_calibration.df')
